...
with
    ...
    rename as (
        select
            ...
            as tripid,
            {{ dbt.safe_cast("vendorid", api.Column.translate_type("integer")) }}
            as vendorid,
            ...
            {{ dbt.safe_cast("dolocationi", api.Column.translate_type("integer")) }}
            ...
            coalesce(
                {{
                    dbt.safe_cast(
                        "payment_type", api.Column.translate_type("integer")
                    )
                }},
                0
            ) as payment_type
